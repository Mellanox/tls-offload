...
static void esp_output_udp_encap(struct xfrm_state *x, struct sk_buff *skb, int tailen)
{
	int encap_type;
	struct udphdr *uh;
	__be32 *udpdata32;
...
/* FIXME: 7 parameters are too many! */
int esp_output_head(struct xfrm_state *x, struct sk_buff *skb, __u8 proto, int tfclen, int tailen, int plen, bool *inplace)
{
	...
	if (x->encap)
		esp_output_udp_encap(x, skb, tailen);
...
/* FIXME: 6 parameters are too many! */
